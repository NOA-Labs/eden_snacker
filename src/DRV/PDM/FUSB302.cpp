...
//  ESP_LOGD("FUSB302", "Port %d SRC vconn_enabled %d cc_polarity %d", port, state[port].vconn_enabled, state[port].cc_polarity);
...
static void fusb302_detect_cc_pin_sink(int port, int *cc1, int *cc2)
{
	...
	int bc_lvl_cc1;
	int bc_lvl_cc2;
//	ESP_LOGD("FUSB302", "Port %d SINK vconn_enabled %d cc_polarity %d", port, state[port].vconn_enabled, state[port].cc_polarity);
	/*
	 * Measure CC1 first.
	 */
	tcpc_read(port, TCPC_REG_SWITCHES0, &reg);
...
{
//  ESP_LOGD("FUSB302", "Port %d pulling_up %d", port, state[port].pulling_up);
	if (state[port].pulling_up) {
		/* Source mode? */
...
static int fusb302_tcpm_set_cc(int port, int pull)
{
  ...
  ESP_LOGD("FUSB302", "Port %d vconn %d cc_polarity %d pulling_up %d pull %d", port, state[port].vconn_enabled, state[port].cc_polarity, state[port].pulling_up, pull);
//  ESP_LOGD("FUSB302", "Port %d cc_polarity %d", port, state[port].cc_polarity);
//  ESP_LOGD("FUSB302", "Port %d pulling_up %d pull %d", port, state[port].pulling_up, pull);
//  ESP_LOGD("FUSB302", "Port %d rx_enable %d", port, state[port].rx_enable);
	switch (pull) {
	case TYPEC_CC_RP:
      /* turn off toggle */
...
//  ESP_LOGD("FUSB302", "Port %d vconn %d", port, state[port].vconn_enabled);
//  ESP_LOGD("FUSB302", "Port %d cc_polarity %d polarity %d snk_sel_pin %d", port, state[port].cc_polarity, polarity, usb_pd_snk_sel_pin);
//  ESP_LOGD("FUSB302", "Port %d pulling_up %d", port, state[port].pulling_up);
//  ESP_LOGD("FUSB302", "Port %d rx_enable %d", port, state[port].rx_enable);
...
static int fusb302_tcpm_set_vconn(int port, int enable)
{
  ...
  ESP_LOGD("FUSB302", "Port %d vconn %d enable %d", port, state[port].vconn_enabled, enable);
//  ESP_LOGD("FUSB302", "Port %d cc_polarity %d", port, state[port].cc_polarity);
//  ESP_LOGD("FUSB302", "Port %d pulling_up %d", port, state[port].pulling_up);
//  ESP_LOGD("FUSB302", "Port %d rx_enable %d", port, state[port].rx_enable);
...
{
	int reg;
//  ESP_LOGD("FUSB302", "Port %d power_role %d data_role %d", port, power_role, data_role);
...
//  ESP_LOGD("FUSB302", "Port %d cc_polarity %d", port, state[port].cc_polarity);
//  ESP_LOGD("FUSB302", "Port %d pulling_up %d", port, state[port].pulling_up);
//  ESP_LOGD("FUSB302", "Port %d rx_enable %d enable %d cc_polarity %d vconn_enabled %d", port, state[port].rx_enable, enable, state[port].cc_polarity, state[port].vconn_enabled);
...
static int fusb302_tcpm_get_message(int port, uint32_t *payload, int *head)
{
  ...
  {
//    ESP_LOGD("FUSB302", "Port %d fifo is empty", port);
		return EC_ERROR_UNKNOWN;
  }
    ...
    if (port == 1) {
//      ESP_LOGD("FUSB302", "p%d Buf[0] = 0x%02X", port, buf[0]);
//      ESP_LOGD("FUSB302", "p%d Buf[1] = 0x%02X", port, buf[1]);
//      ESP_LOGD("FUSB302", "p%d Buf[2] = 0x%02X", port, buf[2]);
    }
    ...
    if (port == 1) {
//      ESP_LOGD("FUSB302", "p%d rv %d len from head = %d", port, rv, len);
    }
    ...
    if (port == 1) {
//      ESP_LOGD("FUSB302", "rv %d head TYPE %d CNT %d Buf = ", rv, PD_HEADER_TYPE(*head), PD_HEADER_CNT(*head));
...
//      Serial.println(" ");
	...
	if (!rv) {
		...
		if (PACKET_IS_GOOD_CRC(*head)) {
			rv = EC_ERROR_UNKNOWN;
//      ESP_LOGD("FUSB302", "p%d get GoodCRC package", port);
		} else {
...
static int fusb302_tcpm_get_vbus_level(int port)
{
	...
	/* Read status register */
	tcpc_read(port, TCPC_REG_STATUS0, &reg);
  ESP_LOGD("FUSB302", "p%d TCPC_REG_STATUS0 %d ", port, reg);
...
//      ESP_LOGD("FUSB302", "p%d interrupt %d interrupta %d interruptb %d", port, interrupt, interrupta, interruptb);
    }
  } else {
    if (interrupt > 0 || interrupta > 0 || interruptb > 0) {
//      ESP_LOGD("FUSB302", "p%d interrupt %d interrupta %d interruptb %d", port, interrupt, interrupta, interruptb);
...
//      ESP_LOGD("FUSB302", "Port %d interrupt TCPC_REG_INTERRUPT_COLLISION", port);
    ...
    if (interrupta & TCPC_REG_INTERRUPTA_TX_SUCCESS) {
      //task_set_event(PD_PORT_TO_TASK_ID(port), PD_EVENT_RX, 0);
//      ESP_LOGD("FUSB302", "Port %d interrupt TCPC_TX_COMPLETE_SUCCESS", port);
      pd_transmit_complete(port, TCPC_TX_COMPLETE_SUCCESS);
    }

    if (interrupta & TCPC_REG_INTERRUPTA_RETRYFAIL) {
      /* all retries have failed to get a GoodCRC */
//      ESP_LOGD("FUSB302", "Port %d interrupt TCPC_REG_INTERRUPTA_TX_SUCCESS", port);
      pd_transmit_complete(port, TCPC_TX_COMPLETE_FAILED);
    }
...
//      ESP_LOGD("FUSB302", "Port %d interrupt TCPC_REG_INTERRUPTA_HARDSENT", port);
...
//        ESP_LOGD("FUSB302", "Port %d interrupt TCPC_REG_INTERRUPTA_HARDRESET", port);
        fusb302_pd_reset(port);
...
//      ESP_LOGD("FUSB302", "Port %d interrupt TCPC_REG_INTERRUPTB_GCRCSENT", port);
      if (state[port].rx_enable) {
        //task_set_event(PD_PORT_TO_TASK_ID(port), PD_EVENT_RX, 0);
      } else {
